...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        print("UNext MedMamba Initiated")
        ...
        self.vssm = VSSM(depths=[2, 2, 4, 2], dims=[96, 192, 384, 768], num_classes=6) # Default constructor from MedMamba Official
    ...
    def forward(self, x):
        ...
        # Get 4 feature maps from VSSM (like stage1 to stage4), assume VSSM is updated to return features
        features = self.vssm.forward_backbone(x)  # Modify VSSM to return intermediate features
        t1 = features[0].permute(0, 3, 1, 2).contiguous()
        t2 = features[1].permute(0, 3, 1, 2).contiguous()
        t3 = features[2].permute(0, 3, 1, 2).contiguous()
        out = t3
