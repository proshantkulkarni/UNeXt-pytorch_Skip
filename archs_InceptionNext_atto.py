...
from inceptionnext import inceptionnext_atto
...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        print("UNext InceptionNext Atto Initiated")
        ...
        self.encoder = inceptionnext_atto(pretrained=False, num_classes=0)  # remove classification head
        ...
        self.skip_t2_proj = nn.Conv2d(80, 32, kernel_size=1)
        # self.skip_t1_proj = nn.Conv2d(80, 16, kernel_size=1)
        self.skip_t1_proj = nn.Conv2d(40, 16, kernel_size=1)
    ...
    def forward(self, x):
        ...
        out = self.encoder.stem(x)             # [B, 40, 64, 64]
        out = self.encoder.stages[0](out)     # [B, 80, 32, 32]
        t1 = out
        out = self.encoder.stages[1](out)     # [B, 128, 16, 16]
        t2 = out
        out = self.encoder.stages[2](out)     # [B, 160, 8, 8]
        t3 = out
