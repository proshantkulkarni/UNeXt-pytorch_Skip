...
from ctrans import ChannelTransformer, CCA, get_CTranS_config
...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        self.ctran = ChannelTransformer(
                        ...
                        vis=False,
                        img_size=224,
                        channel_num=[16, 32, 128, 160], 
                        patchSize=[32, 16, 8, 4]
                    )

        self.cca4 = CCA(160, 160)  # for t4
        self.cca3 = CCA(128, 128)  # for t3
        self.cca2 = CCA(32, 32)    # for t2
        self.cca1 = CCA(16, 16)    # for t1
        ...
        self.dblock1 = nn.ModuleList([shiftedBlock(dim=320, num_heads=num_heads[0], mlp_ratio=1, qkv_bias=qkv_bias,
                                           qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate,
                                           drop_path=dpr[0], norm_layer=norm_layer, sr_ratio=sr_ratios[0])])

        self.dblock2 = nn.ModuleList([shiftedBlock(dim=256, num_heads=num_heads[0], mlp_ratio=1, qkv_bias=qkv_bias,
                                                qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        self.decoder2 = nn.Conv2d(320, 128, kernel_size=3, stride=1, padding=1)
        self.decoder3 = nn.Conv2d(256, 32, kernel_size=3, stride=1, padding=1)
        self.decoder4 = nn.Conv2d(64, 16, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        t1_, t2_, t3_, t4_, _ = self.ctran(t1, t2, t3, t4)          # CTRANS OUTPUTS
        ...
        t4_ = F.interpolate(t4_, size=out.shape[2:], mode='bilinear', align_corners=True)
        skip4 = self.cca4(out, t4_)
        ...
        t3_ = F.interpolate(t3_, size=out.shape[2:], mode='bilinear', align_corners=True)
        skip3 = self.cca3(out, t3_)
        ...
        t2_ = F.interpolate(t2_, size=out.shape[2:], mode='bilinear', align_corners=True)
        skip2 = self.cca2(out, t2_)
        ...
        t1_ = F.interpolate(t1_, size=out.shape[2:], mode='bilinear', align_corners=True)
        skip1 = self.cca1(out, t1_)
