...
# import archs_InceptionNext_MLFC
...
def main():
    ...
    # model = archs_InceptionNext_SE_chan.__dict__[config['arch']](config['num_classes'], config['input_channels'], config['deep_supervision']).cuda()

    # model = archs_InceptionNext_SE_spat_chan.__dict__[config['arch']](config['num_classes'], config['input_channels'], config['deep_supervision']).cuda()

    model = archs_InceptionNext_MLFC_DSC.__dict__[config['arch']](config['num_classes'], config['input_channels'], config['deep_supervision']).cuda()

    # model = archs_InceptionNext_MLFC.__dict__[config['arch']](config['num_classes'], config['input_channels'], config['deep_supervision']).cuda()
    ...
    dummy_input = torch.randn(1, config['input_channels'], config['input_h'], config['input_w']).cuda()
    global model_params, model_gflops
    macs, params = profile(model, inputs=(dummy_input,), verbose=False)

    model_params = params / 1e6
    model_gflops = macs / 1e9
    print(f"\nParams: {model_params:.2f} M")
    ...
    with torch.no_grad():
        for input, target, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            torch.cuda.synchronize()
            end_time = time.time()
            gput.update(end_time - start_time, input.size(0))
            ...
            sensitivity_meter.update(sensitivity.item(), input.size(0))
            specificity_meter.update(specificity.item(), input.size(0))
            accuracy_meter.update(accuracy.item(), input.size(0))
