...
from wavelet_pool2d import AdaptiveWaveletPool2d
from learnable_wavelets import SoftOrthogonalWavelet
...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        pywt_haar = pywt.Wavelet('haar')

        learnable_haar1 = SoftOrthogonalWavelet(
            dec_lo=torch.tensor(pywt_haar.dec_lo, dtype=torch.float32, requires_grad=True),
            dec_hi=torch.tensor(pywt_haar.dec_hi, dtype=torch.float32, requires_grad=True),
            rec_lo=torch.tensor(pywt_haar.rec_lo, dtype=torch.float32, requires_grad=True),
            rec_hi=torch.tensor(pywt_haar.rec_hi, dtype=torch.float32, requires_grad=True),
        )

        learnable_haar2 = SoftOrthogonalWavelet(
            dec_lo=torch.tensor(pywt_haar.dec_lo, dtype=torch.float32, requires_grad=True),
            dec_hi=torch.tensor(pywt_haar.dec_hi, dtype=torch.float32, requires_grad=True),
            rec_lo=torch.tensor(pywt_haar.rec_lo, dtype=torch.float32, requires_grad=True),
            rec_hi=torch.tensor(pywt_haar.rec_hi, dtype=torch.float32, requires_grad=True),
        ...
        learnable_haar3 = SoftOrthogonalWavelet(
            dec_lo=torch.tensor(pywt_haar.dec_lo, dtype=torch.float32, requires_grad=True),
            dec_hi=torch.tensor(pywt_haar.dec_hi, dtype=torch.float32, requires_grad=True),
            rec_lo=torch.tensor(pywt_haar.rec_lo, dtype=torch.float32, requires_grad=True),
            rec_hi=torch.tensor(pywt_haar.rec_hi, dtype=torch.float32, requires_grad=True),
        ...
        self.pool1 = AdaptiveWaveletPool2d(wavelet=learnable_haar1, scales=1)
        self.pool2 = AdaptiveWaveletPool2d(wavelet=learnable_haar2, scales=1)
        self.pool3 = AdaptiveWaveletPool2d(wavelet=learnable_haar3, scales=1)
