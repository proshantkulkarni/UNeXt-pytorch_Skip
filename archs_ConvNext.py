...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        print("UNext ConvNext Initiated")
        ...
        # self.t1_proj = nn.Conv2d(128, 16, kernel_size=1)
        # # self.t2_proj = nn.Conv2d(256, 32, kernel_size=1)
        # self.t2_proj = nn.Conv2d(512, 32, kernel_size=1)

        # self.t3_proj = nn.Conv2d(512, 128, kernel_size=1)

        self.t1_proj = nn.Conv2d(256, 16, kernel_size=1)
        self.t2_proj = nn.Conv2d(512, 32, kernel_size=1)
        self.t3_proj = nn.Conv2d(512, 128, kernel_size=1) 
    ...
    def forward(self, x):
        ...
        # ConvNeXt Encoder
        x = self.convnext.downsample_layers[0](x)       # stem

        x = self.convnext.stages[0](x)                  # stage 1
        t1 = x = self.convnext.downsample_layers[1](x)

        x = self.convnext.stages[1](x)                  # stage 2
        t2 = x = self.convnext.downsample_layers[2](x)

        x = self.convnext.stages[2](x)                  # stage 3
        t3 = x
        ...
        out = torch.add(out, self.t3_proj(t3))
        _,_,H,W = out.shape
        out = out.flatten(2).transpose(1,2)
        ...
        out = torch.add(out, self.t2_proj(t2))
